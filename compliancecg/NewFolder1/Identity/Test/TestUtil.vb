...
Namespace Identity.Test
    Module TestUtil
        ...
        Function CreateManager(ByVal db As DbContext) As UserManager(Of UserAccount)
            Dim options = New IdentityFactoryOptions(Of UserManager(Of UserAccount)) With {
        ...
        Function CreateManager() As UserManager(Of UserAccount)
        ...
        Async Function CreateManager(ByVal context As OwinContext) As Task
            ...
            Dim middleware = New IdentityFactoryMiddleware(Of UserManager(Of UserAccount), IdentityFactoryOptions(Of UserManager(Of UserAccount)))(Nothing, options)
            Await middleware.Invoke(context)
        End Function
    End Module

    Public Class TestProvider
        Inherits IdentityFactoryProvider(Of UserManager(Of UserAccount))

        Public Sub New(ByVal db As DbContext)
            OnCreate = (Function(options, context)
                            Dim manager = New UserManager(Of UserAccount)(New UserStore(Of UserAccount)(db))
                            manager.UserValidator = New UserValidator(Of UserAccount)(manager) With {
                            ...
                            '    manager.UserTokenProvider = New DataProtectorTokenProvider(Of UserAccount)(options.DataProtectionProvider.Create("ASP.NET Identity"))
                            'End If
